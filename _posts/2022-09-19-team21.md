...
We will investigate different applications of meta-RL, specifically the PEARL algorithm. Previous investigations on new environments have been centered on learning unseen tasks. In this project, we will explore new environments by introducing another agent…
We will use the PEARL meta-RL algorithm used in these papers [some papers, remember to include reference to the end of the file]
We will use Derkgym. We will first train some strategies against an agent that only performs random actions. Then we will expose the agent to 1) new opponents; 2) new environments. Using meta-RL, we anticipate that the agent will be able to adapt to new opponents and new maps quicker
It is possible that we encounter a problem where the agent takes too long to learn new opponents and new maps, or if the agent is at all able to learn strategies against the random opponent. Our solution might be…

https://gym.derkgame.com/
http://docs.gym.derkgame.com/#examples
https://github.com/MountRouke/Randos
