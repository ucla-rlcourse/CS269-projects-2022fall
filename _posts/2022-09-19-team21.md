...
In this project, we use the PEARL algorithm written by Kate Rakelly, et al. as the basis of our code, and changed the environment from MuJoCo -- a single agent environment -- to Derk's Gym -- a multi-agent evnrionment. In Derk's gym, there are 3 agents in the home team and 3 agents in the opposing team. For each agent, the observation space is 64; some of these observations include whether an agent possesses a certain ability, whether an opposing agent possesses a certain ability, and position on the map. For each agent, the action space is 5; some of these actions include rotating, moving, and using any of three abilities. To enable multi-agent learning, we will have one shared policy controlling the actions of 3 different agents (the actor network, critic network, will take in observations and latent variable of all 3 agents concatenated together).

In MuJoCo, the agent trained using the PEARL algorithm was able to successfully learn new tasks quicker than other RL methods. For example, a half cheetah trained to move in one velocity was able to learn to run in another target velocity during testing. 
