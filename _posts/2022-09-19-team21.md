...
$$ L_{actor} = E_{s \sim B, a \sim \pi_{\theta}, z \sim q_{\phi}(z|c)} \left[D_{KL}\left(\pi_{\theta}(a|s,z) \|\| \frac{\exp{Q_{\theta}(s,a,z)}}{Z_\theta (s)}\right)\right] \\ $$

$$ \phi_{V} \leftarrow \phi - \alpha \grad_{\phi} \sum_{i} (L_{critic}^{i} + L_{KL}^{i} \\ $$

$$ \theta_{\pi} \leftarrow \theta_{\pi} - \alpha \grad_{\theta_{\pi}} \sum_{i} (L_{actor}^{i} \\ $$

$$ \theta_{Q} \leftarrow \theta_{Q} - \alpha \grad_{\theta_{Q}} \sum_{i} (L_{critic}^{i} \\ $$
