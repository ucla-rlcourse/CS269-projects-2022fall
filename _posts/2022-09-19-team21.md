...
The PEARL algorithm is built on top of the soft actor-critic algorithm (SAC), which is an off-policy method that augments the discounted returns with the policy entropy. The critic consists of two q-value networks ($Q_{\theta}$) and one value network ($V_{\phi}$), and the actor consists of one policy function ($\pi_{\theta}$). As mentioned earlier, the novelty of PEARL is that these actor-critic networks also take in the latent variable returned by the inference network as input, so that they are conditioned by the context posterior. We optimize the parameters of the value network using the mean-squared error loss between... We optimize the parameters of the q-value networks using the mean-squared error loss between... and the ADAM optimizer. We optimize the paramaters of the policy network using... The Adam optimizer is used for all the network updates.
