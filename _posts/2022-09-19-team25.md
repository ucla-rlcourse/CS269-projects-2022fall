...
![ActorLoss]({{ '/assets/images/team23/actor_loss.png'|relative_url}})
...
![CriticLoss]({{ '/assets/images/team23/critic_loss.png'|relative_url}})
{: style="width: 400px; max-width: 100%;"}
...
![EMAC]({{ '/assets/images/team23/emac.png'|relative_url}})
{: style="width: 400px; max-width: 100%;"}
...
![TripletLoss]({{ '/assets/images/team23/tripletloss.png'|relative_url}})
{: style="width: 400px; max-width: 100%;"}
...
```
	'''
	joint_action = list for each agent tuple(agent, action)
	'''
  def step_(self, joint_actions):
    done = False
    info = {agent:{"complete":0, "group_uncover":0, "individual_uncover":0, "visitation_penalty":0, "collision_penalty":0} for agent in self.agents} # Need to set rewards specifically

    for agent, action in zip(self.agents, joint_actions):
      action = action.item()
      if not self.is_valid_(agent, action): # Don't move
      ...
      #turn o to 1 if visited, keep track of what has been observed
      if self.env_done_(): # If the entire grid has been observed
        done = True
    ...
    final_uncovered_ratio = self.calc_uncovered_ratio_() # There is a less bad way of doing this but I'm already too deep
    ...
    for agent in self.agents:
      #updating single agent's reward
