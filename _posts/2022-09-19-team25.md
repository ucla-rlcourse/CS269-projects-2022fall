...
![Env]({{ '/assets/images/team23/emac.png'|relative_url}})
{: style="width: 400px; max-width: 100%;"}
...
![Env]({{ '/assets/images/team23/tripletloss.png'|relative_url}})
{: style="width: 400px; max-width: 100%;"}
...
## Computation Process
EMAC is intended to support multiple rollout threads for statbiltiy. We wanted to solve EMAC using Independent Actor-Critic (IAC) framework, which is an extension of the actor-critic approach to multiple agents where each agent learns independently, each with it's own actor and critic. However, IAC has been problematic to support multiple rollout threads. Thus, we based off a modified Advantage Actor-Critic (A2C) architecture to support multiple rollout threads and attempt to that framework to solve the EMAC algorithm.
### Environment
### Reward
...
Computed in step_() updated in training loop

### Agent
### Networks
We firstly set up the encoder which is an embedding layer. Since different observations from different agents might be in various sizes, the embedding layer enables us to take dynamic input of observations and scale it into fixed length vector so they are consistent in size. This step ensures the uniformity of data uniform which is later feed into the actor network.
...
class Encoder(nn.Module):
  ...
  def forward(self, obs):
    if len(obs.size()) == 3: # When there are multiple agents, change the demension since the shape of the tensor changed as we have 3 agents now
      obs = torch.flatten(obs, start_dim=1) 
    else: # When there is a single agent, the shape of tensor is 1 x size of observation
...
Critic network is a shared centralized critic to estimate value fuction $V(St,ø)$,  critic parameters $ø$ are updated by mini-batch gradient descent. We used Multilayer Perceptron (MLP) which is a feedforward artificial neural network that generates a set of outputs from a set of inputs, the network is trained using backpropogation by MLP.

```
class CriticNetwork(nn.Module):
  #defintes archeteture of network
  #accpects states which are always fixed
  def __init__(self, state_size, hidden_units=100):
    super(CriticNetwork, self).__init__()
    state_size = np.prod(state_size) #Let the network know how large the state is
    ...
    ) #Output a predicted Q value which is an estimated value of state


  def forward(self, state):
    #how the information is fed thru the architecture
    if len(state.size()) == 3:
      state = torch.flatten(state, start_dim=1)
    else:
      state = torch.flatten(state)

    value = self.critic(state) # Passes state through the architecture
    return value 
```
