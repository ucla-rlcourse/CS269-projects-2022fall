...
I evaluate the performance of the Decision Transformer against other state-of-the-art model-free methods using the OpenAI Gym. The hyperparameters used are tuned using a linear scan and suggested by the authors as follows. 
...
Lastly, we must also be wary of the potential errors a transformer can make. The authors also mention that we must avoid datasets that add destructive bias as well as data augmentation with unreliable data. For example, since the DT creates behaviors by conditioning on desired returns, actors that are unstable could lead to unintended results.
