...
Furthermore, as seen in the Cheetah plot, the increase in performance has plateaued and peaked at around 45.0 meaning that it is already state-of-the-art.
...
Several extensions proposed by the authors include utilizing a more sophisticated embeddings for returns, states, and actions i.e. modeling stochastic settings instead of deterministic returns through conditioning on return distributions. In addition, an alternative approach could be to employ transformers to model the state evolution of a trajectory.

Lastly, we must also be wary of the potential errors a transformer can make. The authors also mention that we must avoid datasets that add destructive bias as well as data augmentation since the data souce could be unreliable. For example, since the DT creates behaviors by conditioning on desired returns, actors that are unstable could lead to unintended results.
