...
We have also attempted to add a softmax layer after the weight-selection model to ensure each output weight is positive and sum as 1, but it does not produce better results. 
...
|PPO Specialized Ensemble    |303.566|0.72|298.699|
|Average Generalized Ensemble|N/A    |N/A |228.523|
|PPO Generalized Ensemble    |271.574|0.59|274.682|
...
For the PPO generalized ensemble, the model achieves 517.365 reward.
...
For the PPO specialized ensemble, the model achieves 522.370 reward.
...
Because the original weight output from the model is too jittery, we use a exponential moving average to smooth the weights. The alpha used in moving average is 1/24. 
...
We only see that the 3-lane mid-traffic model is used the least. 
...
2. When the car is sourrounded by traffic after the T-section, we see the heavy-traffic models are given more weight. 
3. The turn into the roundabout is heavily driven by the 4-lane heavy traffic model, so we see the car corrects into the middle lane for the turn. 
4. No specific patterns are found at the exit of roundabout and subsequent driving; the weights are mostly equal. 

For the scenario with 4 lanes and 0.05 traffic, we see the car finishes with reward of 518.216. 
![weight-vis-4-05]({{ '/assets/images/team11/weight-vis-4-05.gif' | relative_url }})
{: style="width: 1000px; max-width: 100%;"}

With less traffic, we can see the weight pattern for turning to be same as above, but we do not see the traffic decision pattern. 
...
For the scenario with 2 lanes and 0.05 traffic, we see the car failed and have only 87.852 reward, and the failure trajectory is about the same as the 2-2 scenario. 
![weight-vis-2-05]({{ '/assets/images/team11/weight-vis-2-05.gif' | relative_url }})
{: style="width: 1000px; max-width: 100%;"}

For the above 2 scenarios, we can see the cars have hit the mid-line. Before the crash, we can see that the 4-lane models have a slightly higher weight than 2-lane models. It is probably the 4-lane model tries to avoid the exit and go back to the middle of the imaginary 4-lane road, which crashes the vehicle into the midline. 
