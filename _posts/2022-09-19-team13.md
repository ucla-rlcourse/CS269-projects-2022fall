...
At local level, CoPO takes Individual rewards $$r_{i, t}^I$$ and Neighborhood reward $$r_{i, t}^N$$ into account. The neighborhood is defined as follow:
...
Local Coordination utilized a *Local Coordination Factor* $$\phi$$ (LCF), inspired by a social psychology metric named social value orientation [7], to coordinated the Individual Reward and the Reward of the its Neighborhood. LCF is a degree range from -90 degree to 90 degree. It coordinated the Individual Reward and Global Reward as follows:
...
Then the PPO framework is utilized to update the policies to maximize coordinated reward.  The coordinated objective is defined as follow:
...
The LCF here will affect agent’s preference of being selfish ($$\phi = 0°$$) or selfless ($$\phi = 90°$$). However, there is still a serious problem that how can we determine the LCF? It is always difficult to find the best value manually which requires a lot of parameter tuning on the training process each time.

The CoPO designs a Global Coordination to adjust LCF as a learnable parameter. At Global Coordination, they design a global reward, which is the sum of the reward of all agents at all steps and *the global objective* is defined as follow:
...
We installed CoPO and setup GPU-supported enviroment in Linux according to [https://github.com/decisionforce/CoPO](https://github.com/decisionforce/CoPO).



### Training & Evaluation

Due to the limitations of memory and computational resources on our PC, we modified the training mode to local, using single thread and removing grid search, following the instructions given in the FAQ on CoPO's GitHub page. We trained CoPO in *Intersection* and *Roundabout* environment, following the example modified code for IPPO local training with GPU acceleration. We also modified the number of agents to 20 for accelerating the model performance on our PC.
...
The trained models are evaluated on 20 episodes and their success rate are **85.67%** in *Intersection* environment and **88.07%** in *Roundabout* environment.

### Visualization

We also visualized the behaviors of the populations during training process to observe their learning processes.

At first, these agents appear to be basically have no idea how to drive.

![r1]({{ '/assets/images/team13/r1.gif' | relative_url }})

![i1]({{ '/assets/images/team13/i1.gif' | relative_url }})

Gradually, they learned how to drive in simple scenarios but still overwhelmed by the complex ones.
...
Firstly, we evaluate the impact of number of agents. The training curve on *Intersection* Environment and *Roundabout* Environment are shown as follows.
