...
CoPO[6] is multi-agent reinforcement learning algorithm on top of MetaDrive. With CoPO, we can speed up vehicles in a multi-agent environment and we shift our focus to the collective speed and safety of all vehicles in the environment. Note that each vehicle is an agent and is put in the same the MetaDrive environment. Under the MetaDrive environment, we looked into the reward function and adjust the reward for speed. We updated reward function in order to obtain an agent with faster speed and relatively high safety.
...
1. Use the functions provided by Rllib and Tune, which are python packages for reinforcemnet agent training. After that, we could train the IPPO trainer in the roundabout environment environment with 1,000,000 steps. 1,000,000 steps are sufficient for the model to peak at the highest success rate.
...
| speed_reward=0.18, success_reward=15.0, penalties=5.0                  | 0.697           | 60.67                 |
...
| speed_reward=0.18, success_reward=15.0, penalties=5.0                  | 0.742           | 28.1                  |
...
| speed_reward=0.18, success_reward=15.0, penalties=5.0                  | 0.69            | 16.34                 |
