...
### DDPG
The deterministic policy gradient (DPG) is based on continuous and deterministic policy hypothesises $$\mu_{\theta}(s): \mathcal{S} \rightarrow A$$ (in comparison with $\pi_{\theta}(s\mid a)$ which is a probability distribution). Instead of using (1), the objective gradient of DPG yields:
