...
### DDPG
The deterministic policy gradient (DPG) is based on continuous and deterministic policy hypothesises $\mu_{\theta}(s): S \rightarrow A$ (in comparison with $\pi_{\theta}(s | a)$ which is a probability distribution). Instead of using (1), the objective gradient of DPG yields:
